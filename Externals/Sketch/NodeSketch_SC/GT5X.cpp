...
uint16_t GT5X::get_cmd_response(uint32_t *params)
{
    ...
    while ((uint32_t)(millis() - last_read) < GT5X_DEFAULT_TIMEOUT)
    {
        ...
        case GT5X_STATE_READ_DEVID:
        {
            ...
            GT5X_DEBUG_PRINT("[+]ID: 0x");
            GT5X_DEBUG_HEXLN(devid);
        ...
        case GT5X_STATE_READ_PARAMS:
            ...
            GT5X_DEBUG_PRINT("[+]Params: 0x");
            GT5X_DEBUG_HEXLN(*params);
        ...
        case GT5X_STATE_READ_RESPONSE:
        {
            ...
            GT5X_DEBUG_PRINT("[+]Response code: ");
            GT5X_DEBUG_DECLN(rcode);
            break;
        }
        case GT5X_STATE_READ_CHECKSUM:
        {
            ...
            uint16_t chksum = GT5X_CMD_START_CODE1 + GT5X_CMD_START_CODE2 + (uint8_t)GT5X_DEVICEID + (GT5X_DEVICEID >> 8);
...
uint16_t GT5X::get_data_response(uint8_t *data, uint16_t len, Stream *outStream)
{
    ...
    while ((uint32_t)(millis() - last_read) < GT5X_DEFAULT_TIMEOUT)
    {
        ...
        case GT5X_STATE_READ_DATA:
        {
            ...
            {
                state = GT5X_STATE_READ_CHECKSUM;
                GT5X_DEBUG_PRINT("[+]Read len: ");
                GT5X_DEBUG_DECLN(len);
            }
        ...
        case GT5X_STATE_READ_CHECKSUM:
        {
            ...
            if (outStream == NULL)
            {
                uint16_t chksum = GT5X_DATA_START_CODE1 + GT5X_DATA_START_CODE2 + (uint8_t)GT5X_DEVICEID + (GT5X_DEVICEID >> 8);
