...
class ProgramBranch(models.Model):
    ...
    ProgramBranch_Name = models.CharField(
        ...
        help_text=
        "Declare proper names according to the typical standards in relation to Program Name."
    )
...
class UserDataCredentials(AbstractUser):
    ...
    middle_name = models.CharField(
        ...
        blank=True,
        help_text=
        "A Name that is not supplied by User Model. Added for Unique User Purposes."
    )
    unique_id = models.UUIDField(
        ...
        unique=True,
        help_text=
        "A Unique Identifier for your Account. This is used for DB authentication and references. Please DO NOT SHARE IT OUTSIDE."
    )
    ...
    dept_residence = models.ForeignKey(
        ...
        help_text=
        'Please refer the program code form where this staff resides.',
        null=True,
        blank=True,
    ...
    def __str__(self):
        return '%s | %s %s %s | %s' % (self.username, self.first_name,
...
class DeviceInfo(models.Model):
    ...
    Device_Name = models.CharField(
        ...
        help_text=
        'Please indicate. We need to know what kind of device is it gonna be to communicate with the server.',
    ...
    Device_Unique_ID = models.UUIDField(
        ...
        help_text=
        "A Unique Identifier for the Device. This is used classroom assignment unique identity of the device."
    )
    ...
    Device_IP_Address = models.GenericIPAddressField(null=False,
                                                     blank=False,
                                                     protocol='ipv4',
                                                     unique=True)
...
class Classroom(models.Model):
    ...
    Classroom_Name = models.CharField(
        ...
        help_text=
        'Name of the classroom. Can be same from any other classrooms as long as they are placed differently and uniquely.',
        validators=[MinLengthValidator(5),
    ...
    Classroom_CompleteString = models.CharField(
        ...
        help_text=
        "This field is automatically filled when you submit it. Putting any value result to it, being discarded.",
        validators=[MinLengthValidator(6),
    ...
    Classroom_Unique_ID = models.UUIDField(
        ...
        help_text=
        "A Unique Identifier for the classrooms. Required for unique identity for the same subject instance with different properties."
    )

    def __str__(self):
        return '%s — %s | %s' % (self.Classroom_CompleteString,
                                 self.get_Classroom_Type_display(),
                                 self.Classroom_Name)

    def save(self, *args, **kwargs):
        self.Classroom_CompleteString = 'Q-%s%s%s' % (
            self.Classroom_Building, self.Classroom_Floor,
    ...
    def clean(self):
        if Classroom.objects.filter(
                ...
                Classroom_Type=self.Classroom_Type).exists():
            try:
                Classroom.objects.filter(
                    Classroom_Building=self.Classroom_Building,
                    Classroom_Floor=self.Classroom_Floor,
                    Classroom_Number=self.Classroom_Number,
                    Classroom_Type=self.Classroom_Type).update(
                        Classroom_CompleteString=self.Classroom_CompleteString)
...
class Course(models.Model):
    ...
    Course_Name = models.CharField(
        ...
        help_text=
        'Please indicate. We need to know what type of room is it gonna be.',
        validators=[MinLengthValidator(5),
                    MaxLengthValidator(100)])
    Course_Code = models.CharField(
        ...
        help_text=
        'Provide Course Code with Probable In Relation to Course itself.',
        validators=[MinLengthValidator(4),
                    MaxLengthValidator(10)])
    Course_Units = models.PositiveIntegerField(
        verbose_name='Course Units',
        help_text=
        'Units is indenpendent and user should know it based on lecture time and classroom type to use.',
        validators=[MinValueValidator(1),
                    MaxValueValidator(6)])
    Course_Capacity = models.PositiveIntegerField(
        ...
        help_text=
        'The number of students that can included in the class. Minimum is 15 and Maximum is 45',
        validators=[MinValueValidator(15),
                    MaxValueValidator(60)])  # Capacity of Room for Students
    Course_Type = models.CharField(
        ...
        help_text=
        'Course Type is dependent, if it needs Laboratory or Technological Only. Please pick properly.',
        validators=[MinLengthValidator(13),
                    MaxLengthValidator(29)],
        default=CourseSessionTypes[0],
        choices=CourseSessionTypes)
    ...
    def __str__(self):
        return '{0} — {1} | {2}'.format(self.Course_Code, self.Course_Name,
                                        self.get_Course_Type_display())
...
class SectionGroup(models.Model):
    ...
    Section_Program = models.ForeignKey(
        ...
        help_text=
        'Please refer the program code form where the section resides.',
        null=False,
        blank=False,
        on_delete=models.CASCADE)
    ...
    Section_SubUniqueGroup = models.CharField(
        ...
        help_text=
        'Subunique Group that distinct other section from one another.',
        null=False,
        blank=False,
    ...
    Section_CompleteStringGroup = models.CharField(
        ...
        help_text=
        'Combined Information into one Codename. This cannot be modified nor replaced. This is submit-dependent content value.',
        null=False,
        blank=False,
        unique=True,
    ...
    def save(self, *args, **kwargs):
        self.Section_CompleteStringGroup = '%s%s%s%s' % (
            self.Section_Program.ProgramBranch_Code, self.Section_Year,
            self.Section_Semester, self.Section_SubUniqueGroup)
    ...
    def clean(self):
        if SectionGroup.objects.filter(
                ...
                Section_SubUniqueGroup=self.Section_SubUniqueGroup).exists():
            try:
                SectionGroup.objects.filter(
                    Section_Program=self.Section_Program,
                    Section_Year=self.Section_Year,
                    Section_Semester=self.Section_Semester,
                    Section_SubUniqueGroup=self.Section_SubUniqueGroup).update(
                    )
...
class CourseSchedule(models.Model):
    ...
    CourseSchedule_CourseReference = models.ForeignKey(
        ...
        help_text=
        'Refers to a candidated subject from which allocates the time.',
    ...
    CourseSchedule_Availability = models.CharField(
        ...
        help_text=
        "Refers to a schedule being able to access based on professor time.",
        null=True,
        blank=True,
    ...
    def __str__(self):
        return 'Schedule of %s | Handled by %s | Every %s | Time: %s-%s' % (
            ...
            self.CourseSchedule_Instructor, self.CourseSchedule_Lecture_Day,
            self.CourseSchedule_Session_Start, self.CourseSchedule_Session_End)
...
class ClassroomActionLog(models.Model):
    ...
    ActionLevel = models.CharField(
        ...
        help_text=
        'The level of log indication. This only benefit for actions being filtered.',
        max_length=255,
        null=False,
        blank=False,
        choices=LevelAlert)
    ...
    TimeRecorded = models.DateTimeField(
        ...
        help_text=
        "Refers to a time from where the log recorded someone's actions.",
        null=False,
        blank=False,
        editable=False)
