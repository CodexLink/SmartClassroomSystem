...
# ! Role Class to Database
class UserRoleTypes(models.Model):
    RoleName = models.CharField(max_length=27, null=False, blank=False, choices=RoleDeclaredTypes, default=RoleDeclaredTypes[0])
...
class Classroom(models.Model):
    ...
    Classroom_Type = models.CharField(max_length=29, null=False, blank=False, validators=[MinLengthValidator(13), MaxLengthValidator(29)], default=CourseSessionTypes[0], choices=CourseSessionTypes)
    Classroom_CompleteString = models.CharField(max_length=6, null=True, blank=True, unique=True, validators=[MinLengthValidator(6), MaxLengthValidator(6)], help_text="This field will be automatically filled when you submit it. Putting any value will result to it, discarded.")
...
class SectionGroup(models.Model):
    Section_Program = models.ForeignKey(ProgramBranch, to_field="ProgramBranch_Code", null=False, blank=False, on_delete=models.CASCADE)
    Section_Year = models.PositiveIntegerField(choices=YearBatchClasses, null=False, blank=False)
    Section_Semester = models.PositiveIntegerField(null=False, blank=False, choices=SemClassification)
    Section_SubUniqueGroup = models.CharField(max_length=3, null=False, blank=False, choices=SubSectionUniqueKeys, validators=[MinLengthValidator(3), MaxLengthValidator(3)])
    Section_CompleteStringGroup = models.CharField(max_length=10, null=False, blank=False, unique=True, validators=[MinLengthValidator(1), MaxLengthValidator(5)])
    ...
    def clean(self):
        if self.checkSGDuplicates(self.Section_Program, self.Section_Year, self.Section_Semester, self.Section_SubUniqueGroup):
            raise ValidationError('Error, you''re choosing a section that is already declared! Please choose another one. Or, if you saved it without any pending changes, please ignore this message if it is.')
        else:
...
class CourseSchedule(models.Model):
    ...
    CourseSchedule_Instructor = models.OneToOneField(UserDataCredentials, to_field="Common_ID", null=False, blank=False, on_delete=models.CASCADE)
    CourseSchedule_Room = models.ForeignKey(Classroom, null=False, blank=False, to_field="Classroom_CompleteString", related_name="ClassRoomMainReference", on_delete=models.CASCADE)
    CourseSchedule_Session_Start = models.TimeField(null=True, blank=True, unique=True)
    CourseSchedule_Session_End = models.TimeField(null=True, blank=True, unique=False)
    CourseSchedule_Lecture_Day = models.CharField(max_length=9, null=False, blank=False, default=SessionDaysClassification[0], choices=SessionDaysClassification, unique=True, validators=[MinLengthValidator(6), MaxLengthValidator(9)])

    def __str__(self):
        return 'Schedule of %s | Handled by %s | Every %s | Time: %s-%s' % (self.CourseSchedule_CourseCode, self.CourseSchedule_Instructor, self.CourseSchedule_Lecture_Day, self.CourseSchedule_Session_Start, self.CourseSchedule_Session_End)

    # ! Need more if and else statement here.
    def clean(self):
        if not (self.CourseSchedule_Lecture_Day and self.CourseSchedule_Session_Start and self.CourseSchedule_Session_End):
